...
import re
from functions import toBinary, toDecimal
...
window1, window2 = decimalToBinary(), None
...
while True:
    ...
    if window == window1 and event == "Convert":
        ...
        if values['decimalNum'].isdecimal():
            ...
            window['result'].Update(toBinary(values['decimalNum']))
        ...
        if re.match("^[10]+$", values['binaryNum'] ) != None:
            window['warning'].Update('')
            window['result'].Update(toDecimal(values['binaryNum']))
